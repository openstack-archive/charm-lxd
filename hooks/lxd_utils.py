...
def config_btrfs(dev):
    ...
    if has_storage():
        cmd = ['lxc', 'storage', 'create', 'lxd_btrfs', 'btrfs',
               'source={}'.format(dev)]
        check_call(cmd)
        cmd = ["lxc", "profile", "device", "set", "default", "root",
               "pool", "lxd_btrfs"]
        check_call(cmd)
    else:
        lxd_stop()
...
def config_lvm(dev):
    ...
    if has_storage():
        cmd = ['lxc', 'storage', 'create', 'lxd_vg', 'lvm',
               'source={}'.format(dev)]
        check_call(cmd)
        cmd = ["lxc", "profile", "device", "set", "default", "root",
               "pool", "lxd_vg"]
...
def config_zfs(dev):
    ...
    if has_storage():
        cmd = ["lxc", "storage", "create", ZFS_POOL_NAME, "zfs",
