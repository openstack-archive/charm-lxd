...
def do_snap_installation(source_channel, user='nova'):
    ...
    :param user: the existing user, where config may be stored, default 'nova'
    :type user: str
    """
    ...
    if CompareHostReleases(lsb_release()['DISTRIB_CODENAME']) < 'bionic':
        status_set("blocked", "Can't install snap on less than bionic release")
        return
    ...
    if current_channel is None:
        log("Waiting 1 minute for lxd to settle", INFO)
        time.sleep(60)
        log("Running /snap/bin/lxd.migrate -yes with LXD_PREINST=1", INFO)
        try:
            env = os.environ.copy()
            env["LXD_PREINST"] = "1"
            check_call(["/snap/bin/lxd.migrate", "-yes"], env=env)
        except CalledProcessError as e:
            log("lxd.migrate -yes failed with: {}".format(str(e)))
            sys.exit(1)
        ...
        # Note that the nova-lxd plugin actually runs the snapped lxd directly,
        # and so uses the /var/lib/nova/.config/lxc directory -- i.e. it is run
        # outside of the snap confinement due to snapd bugs around not allowing
        # writes to /var/lib
        home = pwd.getpwnam(user).pw_dir
        ...
        if os.path.isdir(pkg_config_dir) and not os.path.isdir(target_dir):
...
def has_storage(search_for_pool=None):
    try:
        pools = (check_output([lxc_bin(), 'storage', 'list'], env=lxc_env())
                 .decode('utf-8')
                 .splitlines())
